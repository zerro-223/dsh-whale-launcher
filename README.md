...
**单个原生 exe（约 9.9MB）**，GPU 合成渲染、CSS 动画、拖动丝滑。
...
- **启动前自检**：Node.js / DSH 程序文件（自动识别安装位置）/ npm 缓存 / pnpm /
  系统代理 / 运行状态 / **运行权限与更新权限**（安装目录是否可写、是否已提权）
- **端口占用处置**：webPort 被其他程序占用时，展示占用者（PID / 进程名 / 路径），
  可一键结束该进程（二次确认，且会复核 PID 仍在监听目标端口以避免误杀），
  也可直接在界面内修改 Web 端口——不再只给一句「端口被占用」
- **权限自适配**：`npm install -g` 的写入目标（npm 全局前缀）在部分机器上
  对普通用户只读（仅 `Users:(RX)`），此时更新会明确告知目录与原因，
  并提供**「以管理员身份重启启动器」**（`ShellExecuteW(runas)` 提权，
  提权实例自动豁免单实例互斥）；更新/安装前会强制要求先停止 DSH
...
- **更新检测与手动更新**：自动检查 `@deepseek-ai/dsh` 新版本（npm registry），
  ...
  未安装 DSH 时提示「未安装」，可一键执行 `npm install -g` 完成安装- **深色 / 浅色 / 跟随系统主题**：CSS 变量一键切换（三态循环），默认跟随
...
- **插件管理**：窗口内独立视图（header「插件管理」进入，一键返回首页）——
  ...
  环境检查；「检查更新」会同时取该版本的**发布年龄**，1 小时内发布的标注
  「刚发布」并默认不提供一键更新——检查更新走 npm、实际安装走 pnpm，两者
  缓存独立，刚发布的版本存在"看得见装不上"的时间窗；更新失败会自动刷新重试
  一次，仍失败则提示稍后重试（完整报错见 launcher.log）
- **系统托盘**：关闭窗口最小化到托盘（后台保持状态轮询），托盘左键单击
  恢复窗口、右键菜单「打开主窗口 / 退出」；重复启动 exe 只聚焦已有窗口
  （单实例）
- **运行日志面板**：安装/更新等 npm/pnpm 操作的完整输出实时显示在可收起的
  日志面板，**并落盘**到 exe 旁 `launcher.log`（超 2MB 保留尾部 512KB，
  失败后可回溯原始报错）；页脚「web.log」查看 DSH Web 模式日志、
  「启动器日志」查看启动器自身日志
...
|---|---|---|
| 产物 | exe + `_internal\` 60MB | **单个 exe 约 9.9MB** |
| 运行内存 | ~113MB | **~28MB** |
...
1. 下载 exe（本仓库 Release 的产物名为 `DSH-Launcher-<版本>.exe`，可自行重命名，
   例如 `DSH启动器.exe`），放在任意**可写目录**
...
- `junctionPath` / `dDrivePath`：可选。仅在显式填写时作为 DSH 安装位置的候选路径，
  不填则完全自动识别
- `webPort`：可选，DSH Web 服务端口，默认 3080（设置页可改；被占用时首页会
  给出占用者与处置入口）
